...
void DialogNew::update_info() {
    ...
    int wash_input_x = 0;
    int wash_input_y = row;
    int wash_output_x = column + 1;
    int wash_output_y = 1;
    if ((!ui->WashInputXLine->text().isEmpty()) && (!ui->WashInputYLine->text().isEmpty())) {
            wash_input_x = ui->WashInputXLine->text().toInt();
            wash_input_y = ui->WashInputYLine->text().toInt();
    }

    if ((!ui->WashOutputXLine->text().isEmpty()) && (!ui->WashOutputYLine->text().isEmpty())) {
        wash_output_x = ui->WashOutputXLine->text().toInt();
        wash_output_y = ui->WashOutputYLine->text().toInt();
    }

    //check
        if (!( ((wash_input_x == 0) && (wash_input_y >= 1 && wash_input_y <= row)) ||
               ((wash_input_x == column + 1) && (wash_input_y >= 1 && wash_input_y <= row)) ||
               ((wash_input_y == 0) && (wash_input_x >= 1 && wash_input_x <= column)) ||
               ((wash_input_y == row + 1) && (wash_input_x >= 1 && wash_input_x <= column))))
        {
            qDebug() << "input terminal not on the margin";
            ui->pushButtonCheck->setText("wash input terminal not on the margin");
            return;
        }

    if (!( ((wash_output_x == 0) && (wash_output_y >= 1 && wash_output_y <= row)) ||
           ((wash_output_x == column + 1) && (wash_output_y >= 1 && wash_output_y <= row)) ||
           ((wash_output_y == 0) && (wash_output_x >= 1 && wash_output_x <= column)) ||
           ((wash_output_y == row + 1) && (wash_output_x >= 1 && wash_output_x <= column))))
    {
        qDebug() << "output terminal not on the margin";
        ui->pushButtonCheck->setText("output terminal not on the margin");
        return;
    ...
    for (int i = 0; i < input_x.size(); ++i) {
         if ((wash_input_x == input_x[i] && wash_input_y == input_y[i]) ||
             (wash_output_x == input_x[i] && wash_output_y == input_y[i])) {
              qDebug() << "input terminal ChongFu with wash terminals";
              ui->pushButtonCheck->setText("input terminal ChongFu with wash terminals");
              return;
         }
    }

    if ((wash_input_x == output_x && wash_input_y == output_y) ||
            (wash_output_x == output_x && wash_output_y == output_y)) {
        qDebug() << "output terminal ChongFu with wash terminals";
        ui->pushButtonCheck->setText("output terminal ChongFu with wash terminals");
        return;
    }
    ...
    emit parametersReady(column, row, input_x, input_y, output_x, output_y,
                         wash_input_x, wash_input_y, wash_output_x, wash_output_y);
